...
def decode(
    ...
    def get_logits(input_ids, inference_params, cond=None): # added 2024-06-11
        ...
        if not cg or not decoding:
            ...
            logits = model._decoding_cache.run(
                input_ids, position_ids, inference_params.seqlen_offset, cond=cond  # added 2024-06-11
            ).squeeze(dim=1)
    ...
    while not should_stop(sequences[-1], inference_params):
        scores.append(get_logits(sequences[-1], inference_params, cond=cond))   # added 2024-06-11
...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        output = decode(
            input_ids, self, max_length, top_k=top_k, top_p=top_p, min_p = min_p, temperature=temperature, cond=cond, **kwargs
        )
...
def capture_graph(
    model, inference_params, batch_size, max_seqlen, decoding_seqlen=1, mempool=None, n_warmups=2, cond=None    # added
):
